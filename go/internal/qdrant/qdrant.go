...
func (c *Client) GetAllIndexedHashes(collection string, filePaths []string, batchSize int) (map[string]string, error) {
	...
	slog.Info(fmt.Sprintf("Querying Qdrant for %d indexed file paths to find already embedded documents...", totalPaths))
	...
	for i := 0; i < totalPaths; i += batchSize {
		...
		slog.Info(fmt.Sprintf("Querying DB hashes batch %d/%d (paths %d–%d)...", batchNum, totalBatches, i+1, end))
	...
	slog.Info(fmt.Sprintf("Retrieved %d unique indexed file path(s) from Qdrant.", len(pathHashes)))
	return pathHashes, nil
}
...
func (c *Client) DeleteByPaths(collection string, filePaths []string) (int, error) {
	...
	respBytes, status, err := c.doRequest("POST", fmt.Sprintf("/collections/%s/points/delete", collection), deleteBody)
	if err != nil || status != http.StatusOK {
...
func (c *Client) StoreEmbeddings(collection string, chunks []chunk.Chunk, embeddings [][]float32, dryRun, hybrid bool) (int, error) {
	...
	for i, ch := range chunks {
		...
		var vectorData any
		if hybrid {
			vectorData = map[string]any{"dense": emb}
		} else {
			vectorData = emb
		}
	...
	for idx := 0; idx < len(points); idx += upsertBatchSize {
		...
		respBytes, status, err := c.doRequest("PUT", fmt.Sprintf("/collections/%s/points", collection), upsertBody)
		if err != nil || status != http.StatusOK {
...
func (c *Client) QueryPoints(collection string, queryVector []float32, limit int, hybrid bool) ([]SearchResult, error) {
	if limit <= 0 {
		limit = 5
	}
